    ...
    long long minOperations(vector<int>& nums) {
        ...
        for (int i = 0; i < nums.size(); i++) {
            if (nums[i] % 2 == 0) {
                ...
                long long op1 =
                    (ind < e.size()) ? (1LL * e[ind] - nums[i]) / 2 : 1e9;
                long long op2 =
                    (ind > 0) ? (1LL * nums[i] - e[ind - 1]) / 2 : 1e9;
                ans += min(op1, op2);
                ...
                long long op1 =
                    (ind < o.size()) ? (1LL * o[ind] - nums[i]) / 2 : 1e9;
                long long op2 =
                    (ind > 0) ? (1LL * nums[i] - o[ind - 1]) / 2 : 1e9;
                ans += min(op1, op2);
                // cout<<min(op1,op2)<<endl;
            }
        }
